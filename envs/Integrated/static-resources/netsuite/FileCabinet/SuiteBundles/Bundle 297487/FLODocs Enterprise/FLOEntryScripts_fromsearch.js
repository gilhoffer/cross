...
//alert(laststage);
		...
		stage=type;
                if(type=="processframe" | type=="custframe" | type=="custframesearch" | type=="processlist")
                {
                  loadFrame(type);
        ...
        function loadFrame(type)
        {
		     ...
		     if(type=="custframesearch" && document.getElementById("custmapsearch").src)
           {
    			    var fullurl = window.location.href;
    			    var params=fullurl.substring(fullurl.indexOf('&'));
    				console.log(params);
    				document.getElementById("custmapsearch").src="/app/site/hosting/scriptlet.nl?script=customscript_flo_cust_viz_fr_search&deploy=1"+params;
         ...
         function showControls(icontype)
         {
          ...
          else if(icontype=="newframe" | icontype=="processdetail")
          {
           ...
           else if(icontype=="custframe" | icontype=='custframesearch')
          {
		   $(".backbutton").fadeIn(fadespeed);
		   $(".homebutton").fadeIn(fadespeed);
