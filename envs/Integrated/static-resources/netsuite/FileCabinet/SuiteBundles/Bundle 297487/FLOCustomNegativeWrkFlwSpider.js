 ...
 *
 * Inactivates customization record for deleted or inactivated workflows
 */
define(['N/runtime','N/config','N/record','N/search','N/task'],
    function(runtime,config,record,search,task){
    	
		//FLOCustomNegativeWrkFlwSpider
        var WORK_FLOW=10; // workflow
        ...
        */
		function execute(context)
		{
			try {
				...
				var scriptObj = runtime.getCurrentScript();
     			var lastIndex = scriptObj.getParameter({name: 'custscript_flo_negative_wf_lastindex'});
 				if (lastIndex==null)
 				 	lastIndex=1;
				...
				// start main logic
                var activeWrkFlow=[];
                getSearchArray(activeWrkFlow,0);
                getCustomFloRecords(activeWrkFlow,lastIndex,starttime);
                // end main logic
        ...
        /**
        * Gets all workflows customization records and inactivates them when they are not on the list of active workflows
        */
		function getCustomFloRecords(activeRecords, lastIndex, starttime){
            ...
            var customrecord_flo_customization_record = search.create({
                ...
                filters : [ ['custrecord_flo_cust_type', 'anyof', WORK_FLOW],'AND',['isinactive', 'is', 'F'],'AND',['name','doesnotstartwith','prototype customization'],'AND',['internalidnumber', 'greaterthan', lastIndex ]]
            });
        ...
        /**
        * Retrieves all active workflows in the account
        */
        ...
        */
		function rescheduleJob(internalId){
		    ...
		    scheduledScriptTask.params = { custscript_flo_negative_wf_lastindex : internalId }; // set the last index
